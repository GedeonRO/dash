<script lang="ts">
	import { show_edit_subcategory } from '$lib/store';
	...
	async function update() {
        ...
        if(image_input.files?.length===0){
                await fetch(api_url, {
                        method:"PUT",
                        body: JSON.stringify({ id: $subcat_being_edited.id, name, featured }),
                        headers:{
			...
			await fetch('https://api.cloudinary.com/v1_1/dmqdfozqg/image/upload', {
				...
				.then(async (res) => {
					if (res.status === 200) {
						...
						await fetch(api_url, {
							method: 'PUT',
							body: JSON.stringify({ id: $subcat_being_edited.id, name, featured, image, category }),
							headers: {
								Authorization: localStorage.getItem('token')!,
								'Content-Type': 'application/json'
							}
							...
							if (res.status === 409) {
								loading = false;
								alert('Une sous categorie avec le meme nom existe deja');
								return;
							}
							loading = false;
							alert('Une erreur est survenue. Reessayer ou contactez les developpeurs');
	...
	<div class="absolute z-30 flex flex-col w-full h-screen items-center justify-center">
		<div class="shadow-2xl shadow-black h-[350px] w-[700px] rounded-md bg-slate-400 flex flex-col">
			<div class=" flex justify-around items-center p-1 mb-10">
				<button
					on:click={() => {
						show_edit_subcategory.set(false);
					}}
